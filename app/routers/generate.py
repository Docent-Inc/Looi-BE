...
from app.schemas.response.gpt import BasicResponse, ImageResponse
...
async def generate_basic(
    ...
    dream_name, dream, dream_resolution, today_luck, dream_image_url = await generate_text(text, current_user.id, db)
    return ApiResponse(
        success=True,
        data=BasicResponse(
...
async def generate_image(
    ...
    dream_image_url = await create_img(prompt, current_user.id)
